...
const useLayoutLogic = ({ onVisualClick, layoutDef }) => {
  ...
  if (!context) throw new Error('useLayoutLogic must be used within LayoutStateProvider')
  const { visualShapes, setSelectedVisual, initializeLayout, handleEditImage } = context

  React.useEffect(() => { initializeLayout(layoutDef) }, [layoutDef, initializeLayout])
...
const LayoutRenderer = ({ layoutDef, onVisualClick }) => {
  const { visualShapes, handleShapeClick, handleEditImage } = useLayoutLogic({ onVisualClick, layoutDef })
  ...
  const renderElement = (element, index) => {
    ...
    switch (element.type) {
      case 'text':
        return (
          <div key={index} className={`layout-element text-element ${element.styles}`} style={elementStyle}>
            {element.content}
          </div>
        )
      case 'visual':
        return (
          <div key={index} className={`layout-element visual-element ${element.styles}`} style={elementStyle} onClick={(e) => handleShapeClick(e, element)}>
            {visualData?.imageUrl ? (
              <div className="visual-image-container">
                <img src={visualData.imageUrl} alt="Extracted Image" className="visual-image" />
                <div className="edit-overlay">
                  <button className="edit-button" onClick={(e) => { e.stopPropagation(); handleEditImage(visualData.imageUrl) }}>
                    Edit Design
                  </button>
                </div>
              </div>
            ) : (
              <div className="visual-placeholder"><p>{element.placeholder}</p></div>
            )}
          </div>
        )
      case 'logo':
        return (
          <div key={index} className={`layout-element logo-element ${element.styles}`} style={elementStyle}>
...
export const LayoutSelector = ({ currentLayout, onLayoutChange }) => (
  <div className="layout-selector">
    ...
    <select id="layoutSelect" className="layout-select" value={currentLayout} onChange={(e) => onLayoutChange(parseInt(e.target.value, 10))}>
...
export const LayoutPreview = ({ currentLayout, onVisualClick, registerEventHandler, openDesignVariantEditor }) => {
  ...
  return (
    <div className="layout-preview">
      <LayoutStateProvider registerEventHandler={registerEventHandler} openDesignVariantEditor={openDesignVariantEditor}>
        <LayoutRenderer layoutDef={currentLayoutDef} onVisualClick={onVisualClick} />
      </LayoutStateProvider>
    </div>
  )
}
