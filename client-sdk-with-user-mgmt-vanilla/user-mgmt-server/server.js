...
async function siviPost(endpoint, body) {
  const res = await fetch(`${process.env.SIVI_AUTH_URL}${endpoint}`, {
    method: 'POST',
...
app.post('/delete-sivi-user', async (req, res) => {
  try {
    ...
    const data = await siviPost('/delete-user', { abstractUserId });
    res.json(data);
  } catch (error) {
    console.error('Error:', error);
    res.status(500).json({ error: error.message || 'Internal Server Error' });
...
app.post('/set-credit-limit', async (req, res) => {
  try {
    ...
    const data = await siviPost('/set-user-credit-limit', { abstractUserId, creditLimit });
    res.json(data);
  } catch (error) {
    console.error('Error:', error);
    res.status(500).json({ error: error.message || 'Internal Server Error' });
